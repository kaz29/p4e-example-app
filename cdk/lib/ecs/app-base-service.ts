...
import {LoadBalancerType} from "@aws-cdk/aws-ecs-patterns";

/**
 * The properties for the base LoadBalancedEc2Service or LoadBalancedFargateService service.
 */
export interface AppBaseServiceProps {
  /**
 ...
 *
 * 1.3.0の、aws-ace-patterns/lib/base/load-balances-service-baseをもとに改造
 */
export abstract class AppBaseService extends cdk.Construct {
   ...
   */
  constructor(scope: cdk.Construct, id: string, props: AppBaseServiceProps) {
    super(scope, id);
    ...
    const targetProps = {
      port: props.targetPort ? props.targetPort: 80,
      protocol: props.protocol,
      pathPattern: '*',
      priority: 1,
      healthCheck: {
        path: props.healthCheckPath !== undefined ? props.healthCheckPath: '/',
      },
    };
    ...
    let port = hasCertificate ? 443 : 80;
    if (props.targetPort) {
      port = props.targetPort;
    }

    if (this.loadBalancerType === LoadBalancerType.APPLICATION) {
      this.listener = (this.loadBalancer as ApplicationLoadBalancer).addListener(`PublicListener-${port}`, {
        port,
        protocol: props.protocol,
        open: true
      });

      // デフォルトルールはmodify_ruleできないので、固定レスポンスを返すruleをデフォルトルールとして登録する
      this.listener.addFixedResponse('default', {
        statusCode: '404',
      });
      ...
      this.listener = (this.loadBalancer as NetworkLoadBalancer).addListener(`PublicListener-${port}`, { port: port });
      this.targetGroup = this.listener.addTargets('ECS', targetProps);
    }
